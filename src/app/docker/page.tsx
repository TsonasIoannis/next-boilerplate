  ...
  return (
    <main className="flex flex-col p-24">
      ...
      <p>
        ...
        <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">node:lts-alpine</code>, which is a small but powerful image that
        contains NodeJS. The <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">lts</code> refers to the version of NodeJS. If
        you want to use a different version, you could replace it with any applicable version!
      </p>
      ...
      <p>
        Why is this line here? Well, the <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">node:lts-alpine</code> image is
        based on Alpine Linux, which is a very small Linux distribution.
        However, it is so small that it doesn't have all the libraries that
        <em>some</em> NodeJS packages need. The <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">libc6-compat</code>
      ...
      <p>
        ...
        neat and tidy, we first create a new directory called <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">/app</code>
        and set it as our working directory. Then, we copy over the
        <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">package.json</code>, and <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">package-lock.json</code>, files.
      </p>
      ...
      <p>
        This block of code it will run <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">npm ci</code>.</p>
      ...
      <p>
        ...
        directory to <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">/app</code>. Then, we copy over the
        <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">node_modules</code> folder from our previous image layer. After
        copying the <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">node_modules</code> folder, we copy over the rest of
        our app. This is done in two steps to improve build times. If we copied
        over the entire app first, then every time we made a change to our app,
        we would have to reinstall our dependencies. By copying over the
        <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">node_modules</code> folder first, we can skip the dependency
        installation step if we haven't changed our dependencies! Smart, right?
      </p>

      <p>
        Finally, we run <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">yarn build</code> to execute the command that is
        defined in our <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">package.json</code> file. This command is usually
        <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">next build</code>, but it can be changed to whatever you want. It
        doesn't really matter if we use <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">yarn</code> or <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">npm</code>
        here, because we already installed our dependencies in the previous step
        and the package manager doesn't really matter for the build process!
      </p>
      ...
      <p>
        ...
        actually run our app. This part is a bit longer, so let's go through it
        step by step.
      </p>

      <p>
        Again, we start by creating a new image layer and setting our working
        directory to <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">/app</code>. We then set the <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">NODE_ENV</code>
      ...
      <p>
        ...
        user with <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">USER nextjs</code>.
      </p>

      <p>
        Now we can finally copy over the build artifacts from the previous image
        layer. We first copy over the <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">./public</code> folder which
      ...
      <p>
        We first expose port <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">3000</code> to the network with
        <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">EXPOSE 3000</code>. This doesn't actually do anything, but it's a
        good practice to include it so that other developers know which port the
        Docker Container will be running on. Next, we set the <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">PORT</code>
        environment variable to <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">3000</code>. This is used by NextJS to
        determine which port to run on. Finally, we set the <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">HOST</code>
        environment variable to <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">localhost</code>.
      </p>
      ...
      <p>
        ...
        start it with <code className="text-sm sm:text-base inline-flex text-left items-center bg-gray-700 text-white rounded-lg p-1">node server.js</code>. That's it! We're done! 🎉🎉🎉
      </p>
